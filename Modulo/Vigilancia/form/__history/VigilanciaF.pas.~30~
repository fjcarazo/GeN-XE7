...
begin
 IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'Vigilancia.ini');
  capturandoAVI := false;
  idResultado := 0;
...
procedure TVigilanciaForm.FormClose(Sender: TObject;
...
begin
  ...
  begin
    try
      ...
      imagenAnterior.LoadFromClipBoardFormat(cf_BitMap, ClipBoard.GetAsHandle(cf_Bitmap), 0);
      error := 0;
      ...
      begin
        ...
        begin
         ...
         error := error + abs(imagenActual.Canvas.Pixels[i, j] - imagenAnterior.Canvas.Pixels[i, j]);
        end;
      end;
...
//          sendgmail;
      if IniFile.ReadString('img','path','') <> sendimage then IniFile.WriteString('img','path',sendimage);
      Contador := Contador + 2;
      ShowMessage('foto');
    end;
  ...
  begin
    bDetenerClick(Self);
    MessageDlg('Debe inicar la cámara para capturar el movimiento.', mtWarning, [mbok], 0);
  end;
end;
...
begin
WinExec(PAnsiChar(ExtractFilePath(Application.ExeName)+'\about.exe'),SW_SHOWNORMAL);
end;
