...
procedure TIngresoDineroForm.Quicksort(Grid: TStringGrid;
...
procedure TIngresoDineroForm.Sortgrid(Grid: TStringGrid;
  ...
  if Label13.Visible = True Then
    ...
    begin
      ...
      try
        ...
        begin
          Query.SQL.Text :=
            'INSERT INTO ChequeEntregado (CodCheque,Numero,Detalle,CodFactura,MntCheque,Fecha,Dias) VALUES '
        ...
        For i := 1 to PagoStringGrid.RowCount - 1 do
        begin
          Query.SQL.Text := 'DELETE * FROM Cheque WHERE CodCheque=' +
...
Begin
  ...
  // NUMERO DEL CHEQUE;
  PagoStringGrid.Cells[1, Cuenta] := Query.FieldByName('Detalle').AsString;
  // DETALLE DEL CHEQUE;
  PagoStringGrid.Cells[2, Cuenta] := Query.FieldByName('MntCheque').AsString;
  // IMPORTE DEL CHEQUE
  PagoStringGrid.Cells[3, Cuenta] := Query.FieldByName('Dias').AsString;
  // A CUANTOS DIAS ESTA EL CHEQUE

  PagoStringGrid.Cells[4, Cuenta] := Query.FieldByName('CodFactura').AsString;
  // CODIGO DE FACTURA DEL CHEQUE;
  PagoStringGrid.Cells[5, Cuenta] := Query.FieldByName('Fecha').AsString;
  // FECHA DE EMISION DEL CHEQUE
  PagoStringGrid.Cells[6, Cuenta] := Query.FieldByName('CodCheque').AsString;
  // FECHA DE EMISION DEL CHEQUE
...
procedure TIngresoDineroForm.PagoStringGridDrawCell(Sender: TObject;
