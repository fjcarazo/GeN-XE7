...
begin
   ...
   Pagado := Pagado + StrToFloat(FEContado.Text)+ StrToFloat(FECheque.Text) +
             StrToFloat(FETarjeta.Text) + StrToFloat(FEOtro.Text) + StrToFloat(ChequeTerceroEdit.Text);
   Saldo := Total - Pagado;
...
OperacionDataModule.FormaPago('CtaCte','0','0','0',FloatToStr(FFormaPago.Pagado),FFormaPago.FEContado.Text,'0',FFormaPago.FECheque.Text,FFormaPago.chequenroEdit.Text,FFormaPago.chequedetalleEdit.Text,FFormaPago.chequediaEdit.Text,'0',FFormaPago.tarnroEdit.Text,FFormaPago.FETarjeta.Text,FFormaPago.FEOtro.Text,'0','0','0',FloatToStr(FFormaPago.Saldo),FloatToStr(FFormaPago.Pagado),'0','0',FFormaPago.ChequeTerceroEdit.Text,'0','0','0');
...
if Label13.Visible = True Then if PagoStringGrid.Cells[1,1] <> '' then begin
 ...
 // PASAR LOS DATOS A CHEQUES ENTREGADOS
     ...
     begin
     Query.SQL.Text:='INSERT INTO "ChequeEntregado" (NUMERO, CHEQUE, DESCRIPCION, FACTURA, IMPORTE, FECHA, DIAS) VALUES '+
 ...
 // ELIMINAR CHEQUE DE CARTERA
     For i := 1 to PagoStringGrid.RowCount-1 do
     begin
    Query.SQL.Text:='DELETE * FROM "Cheque" WHERE "Cheque".CODIGO='+PagoStringGrid.Cells[6,i];
    Query.ExecSQL;
    end;
...
begin
   ...
   Query.SQL.Text:='SELECT * FROM "Cheque" WHERE "Cheque".CODIGO='+ChequeCodCheque;
   Query.Open;
   PagoStringGrid.Cells[0,cuenta] := Query.FieldByName('NUMERO').AsString;//NUMERO DEL CHEQUE;
   PagoStringGrid.Cells[1,cuenta] := Query.FieldByName('DESCRIPCION').AsString;//DETALLE DEL CHEQUE;
   PagoStringGrid.Cells[2,cuenta] := Query.FieldByName('IMPORTE').AsString;//IMPORTE DEL CHEQUE
   PagoStringGrid.Cells[3,cuenta] := Query.FieldByName('DIAS').AsString;//A CUANTOS DIAS ESTA EL CHEQUE

   PagoStringGrid.Cells[4,cuenta] := Query.FieldByName('FACTURA').AsString;//CODIGO DE FACTURA DEL CHEQUE;
   PagoStringGrid.Cells[5,cuenta] := Query.FieldByName('FECHA').AsString;//FECHA DE EMISION DEL CHEQUE
   PagoStringGrid.Cells[6,cuenta] := Query.FieldByName('CODIGO').AsString;//FECHA DE EMISION DEL CHEQUE
...
ChequeTerceroEdit.Text:=FloatToStr(StrToFloat(ChequeTerceroEdit.Text) + StrToFloat(PagoStringGrid.Cells[2,i]));//TOTAL
CalculaSaldo;
end;
end;
