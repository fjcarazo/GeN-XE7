...
uses
  ...
  shellapi,
  Dialogs, StdCtrls, ExtCtrls, DataModule, DB, IBCustomDataSet, IBQuery, Buttons;
...
begin
  ...
  then
      ...
      try
        ...
        begin
         XlsWriteCellLabel(FStream, i, 3, t.Fields.Fields[i].DisplayName);
                   ...
                   // Crear los contenidos de cada línea
        ...
        begin
          ...
          XlsWriteCellLabel(FStream, 1, J, 'Nombre nº ' + inttostr ( j ) );  // Nombre
    ...
    then              // Carga la hoja de cálculo recien creada
      begin

        ShellExecute(Handle,nil, PChar( aa ), '', '',SW_SHOWNORMAL) ;

        ShowMessage(
        'La hoja de cálculo ha sido creada y se está cargando.'  ) ;

      end
    else
      begin
        ShowMessage(
        'La hoja de cálculo ha sido creada.' + #13+ #13+
