...
uses
  ...
  Dialogs, DB, ADODB, ComCtrls, StdCtrls, Buttons, ExtCtrls, DataModule, Grids, DBGrids, IBCustomDataSet, IBQuery, ImprimirDM,
...
'  ("CtaCteItem".CUOTA > '+cuota+') AND'+
'  ("CtaCteItem".VENCE >= '+QuotedStr(DateToStr(DTP1.Date))+') AND '+
'  ("CtaCteItem".VENCE <= '+QuotedStr(DateToStr(DTP2.Date))+') AND '+
...
begin
q.sql.Text:='SELECT'+
'  count("CartonCobranza".IMPRESION)'+
' FROM'+
'  "CartonCobranza"'+
' WHERE'+
'  ("CartonCobranza".OPERACION = '+PlanEdit.Text+' ) AND'+
'  ("CartonCobranza".FECHA <= '+QuotedStr(DateToStr(DTP1.Date))+' )'+
'';
q.Open;
if q.Fields[0].AsInteger > 0 then
 ...
 q.Open;
  ...
  begin
   q.SQL.Text:='Insert into "CartonCobranza" (OPERACION, FECHA, IMPRESION, USUARIO) values'+
   '( '+PlanEdit.Text+', '+QuotedStr(DateToStr(DTP2.Date))+', 1, '+Usuario+')';
   Q.ExecSQL;
  end;
 end
 else
