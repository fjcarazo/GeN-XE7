...
begin
  Tabla.SQL.Text := 'SELECT ' + QuotedStr(DM.ConfigQuery.FieldByName('Nombre')
    ...
    '  "RendidoCobrador".CODIGO,        ' + '  "RendidoCobrador".VENTA,        '
    + '  "RendidoCobrador".CUOTA,    ' + '  "RendidoCobrador".FECHA,       ' +
    '  "RendidoCobrador".RENDIDO,      ' + '  "RendidoCobrador".PAGADO,     ' +
    '  "RendidoCobrador".PROCESADO,  ' + '  "Cobrador".NOMBRE    ' +
    ' FROM                   ' + '  "RendidoCobrador"   ' +
    '  INNER JOIN "Cobrador" ON ("RendidoCobrador".COBRADOR = "Cobrador".CODIGO) '
    + ' WHERE                                ' +
    '  ("RendidoCobrador".FECHA >= ' + QuotedStr(DateToStr(DTP1.Date)) +
    ' ) AND' + '  ("RendidoCobrador".FECHA <= ' + QuotedStr(DateToStr(DTP2.Date)
    ) + ' ) AND ' + '  ("RendidoCobrador".COBRADOR = ' +
    QuotedStr(CodigoEdit.Text) + ' )' + '';
  Tabla.Open;
end;
...
begin
  ...
  try
    ...
    begin
      CodigoEdit.Text := BuscarCobradorForm.Tabla.FieldByName('CODIGO')
        .AsString;
      NombreLabel.Caption := BuscarCobradorForm.Tabla.FieldByName
        ('NOMBRE').AsString;
    end;
