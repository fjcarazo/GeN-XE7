...
uses
  ...
  RpBase, RpSystem, RpRender, RpRenderCanvas, RpRenderPrinter, Printers, RPDevice, ImprimirDM,
  IBCustomDataSet, IBQuery, OleCtrls, SHDocVw, OperacionDM;
...
type
 ...
 private
  ...
  CMV, UltCosto, subtotal, Impuesto, NG21, IVA21, NG105, IVA105, NGO, IVAO, desc, costo, reparaciones, Total, IIBB, NGIIBB, Exento, ComisionVendedor :Double;
  CtaNombre,
  CtaTipo,
  CtaAnticipo,
  CtaIIBB,
  code, Dia, Mes, Ano, TDocumento, Tiempo, T2, Precio, ChequeCodCheque,ChequeNumero,ChequeDetalle,ChequeCodFactura,ChequeMntCheque,ChequeFecha,ChequeDias, Fecha, FechaVence, iv, DetalleFactura :string;
  BalanceAnterior, Interes, BalanceTotal, Deuda, Saldo, Pagado, Comision : Double;
  Tipo: Char;
...
FETarjeta.Text:='0';
FEOtro.Text:='0';
while Cuenta > 1 do QuitarBitBtn.Click; //vaciar articulos
//si es venta
if TipoRadioGroup.ItemIndex = 0 then  AgregarBitBtn.Click
 else
//si es Presupuesto
...
begin
 //Calcula los totales de la factura
   ...
   IF SGFact.Cells[11,i] <> '' then UltCosto := UltCosto + StrToFloat(SGFact.Cells[11,i]);// Calcula el Ultimo Costo
...
// escribe los valores en las celdas
   ...
   SGTotal.Cells[1,4] := Format('%8.2n',[Total]);
         ...
         Pagado := StrToFloat(FEContado.Text)+StrToFloat(FECheque.Text)+StrToFloat(FETarjeta.Text)+StrToFloat(FEOtro.Text);
         Saldo:= total - Pagado;
   ...
   try
    ...
    ClienteEdit.Text := FBuscaCliente.Tabla.FieldByName('CODIGO').AsString;
       ...
       begin
        ...
        IF Tabla.FieldByName('PAGARE').AsString = 'SI' THEN PagareCheckBox.Checked := True;
        //IVA
       ...
       begin
        Tabla.Close;
        Tabla.SQL.Text:='SELECT * FROM "CtaCte" WHERE CLIENTE='+ClienteEdit.Text;
        Tabla.Open;
        ...
        begin
         ...
         if Deuda > 0 then SGFact.Cells[1,cuenta] := 'Deuda CtaCte' else SGFact.Cells[1,cuenta] := 'Saldo a Favor'; //descripción
         SGFact.Cells[2,cuenta] := '0';//serie
         SGFact.Cells[3,cuenta] := '1';//cantidad
         SGFact.Cells[4,cuenta] := Format('%8.2f',[Deuda]);//precio
         SGFact.Cells[5,cuenta] := Format('%8.2f',[StrToFloat(SGFact.Cells[4,cuenta]) * StrToFloat(SGFact.Cells[3,cuenta])]);//total
...
begin
 ...
 if (TipoRadioGroup.ItemIndex = 0) and (DM.ConfigQuery.FieldByName('CodigoBarra').AsString = 'SI') then FBuscaArticulo.cb:='si';
  try
   ...
   If FBuscaArticulo.Tabla.Active = True then
    begin
     ...
     Tabla.SQL.Text := 'SELECT * FROM "Articulo" '+
                       'WHERE CODIGO = '+(FBuscaArticulo.Tabla.FieldByName('CODIGO').Asstring);
     Tabla.Open;
     if PrecioLabel.Caption = '0' then PrecioLabel.Caption := '';
     SGFact.Cells[0,cuenta] := FBuscaArticulo.Tabla.FieldByName('CODIGO').Asstring;
     SGFact.Cells[1,cuenta] := Tabla.FieldByName('DESCRIPCION').AsString; //nombre
     SGFact.Cells[2,cuenta] := '0';
     SGFact.Cells[3,cuenta] := '1';//cantidad
     ...
     SGFact.Cells[5,cuenta] := FloatToStr(StrToFloat(SGFact.Cells[4,cuenta]) * StrToFloat(SGFact.Cells[3,cuenta]));//total
     SGFact.Cells[6,cuenta] := FloatToStr(Tabla.FieldByName('TASA').AsFloat);//IVA
     SGFact.Cells[8,cuenta] := FloatToStr(Tabla.FieldByName('COSTO').AsFloat * StrToFloat(SGFact.Cells[3,SGFact.Row]));//PRECIO DE COSTO
     SGFact.Cells[9,cuenta] := Tabla.FieldByName('IIBB').AsString;//ACTIVIDAD DE MONOTRIBUTO
     ...
     SGFact.Cells[11,cuenta] := FloatToStr(Tabla.FieldByName('ULTCOSTO').AsFloat * StrToFloat(SGFact.Cells[3,SGFact.Row]));//PRECIO DE COSTO
    end;
   FBuscaArticulo.Free;
   Cuenta := Cuenta +1;
   Tabla.Close;
...
begin
 ...
 with OperacionDataModule do
  begin
   ...
   ProcOPER('PED',cbTipo.Text, ClienteEdit.Text, FormatDateTime('mm/dd/yyyy hh:mm:ss',FechaDateTimePicker.DateTime),VendedorEdit.Text,'',CtaNombre,FALSE,PagareCheckBox.Checked, costo, Comision,Impuesto,StrToFloat(FECheque.Text),0,StrToFloat(FEContado.Text),Total,subtotal,desc,StrToFloat(FETarjeta.Text),StrToFloat(FEOtro.Text),Saldo,Pagado,Interes,NG105,NG21,IVA105,IVA21,Deuda,UltCosto);
  end;
...
begin
if (DM.ConfigQuery.FieldByName('IVA').AsString = 'RI') then cbTipo.ItemIndex := 1
else cbTipo.ItemIndex := 2;
nuevo;
end;
...
begin
 ...
 try
   BuscarVendedorForm.ShowModal;
      ...
      VendedorEdit.Text:= BuscarVendedorForm.Tabla.FieldByName('CODIGO').AsString;
