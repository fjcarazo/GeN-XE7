...
uses
  ...
  Dialogs, DB, ADODB, Grids, DBGrids, StdCtrls, Buttons, Mask, DBCtrls, DataModule,
  ExtCtrls;
...
var numfact, c, i, OrdTrans : integer;
...
if FPagoOK = True then begin
     ShortDateFormat := 'dd/mm/yyyy';
     Fecha := DateToStr(now);
  // Obtener el numero de linea
     QTemp.SQL.Text := 'Select Max(Linea) From Pagos where CodImpuestoServicio = '+Codigo;
     QTemp.Open;
     If QTemp.RecordCount = 0 then Linea:=1
     else Linea := QTemp.Fields[0].AsInteger+1;
     QTemp.Close;
  ...
  // Insertar en la tabla de Pagos
          begin
          QTemp.SQL.Text := 'Insert Into Pagos (CodImpuestoServicio, Linea, Fecha, Hora, Detalle, Importe) Values'+
          '( '+Codigo+','+IntToStr(Linea)+','+QuotedStr(fecha)+','+QuotedStr(TimeToStr(Time))+',''Entrega de Dinero'','+Pagado+')';
          QTemp.ExecSQL;
          end;
  ...
  // Insertar en la tabla de Cheque
          if NroCheque <> '' then
          begin
           QTemp.SQL.Text := 'Insert Into Cheque (Numero, Detalle, MntCheque, Fecha, Dias) Values'+
  ...
  // Insertar en la  Caja
          ...
          begin
           ...
           ' ('+QuotedStr('Cheque')+', '+MntCheque+', '+QuotedStr(fecha)+', '+QuotedStr(TimeToStr(Time))+')';
           QTemp.ExecSQL;
           end;
  ...
  // Insertar en la  Caja
          ...
          begin
           QTemp.SQL.Text := 'Insert Into Caja (Detalle, Haber, Fecha, Hora) Values'+
           ' ('+QuotedStr('Efectivo')+', '+MntContado+', '+QuotedStr(fecha)+', '+QuotedStr(TimeToStr(Time))+')';
           QTemp.ExecSQL;
           end;
...
//--CONTABILIDAD
    ...
    //Insertar en la tabla LibroDiario
        ...
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(fecha)+', '+QuotedStr('PAGO DE SERVICIO '+(ProveedorLabel.Caption))+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', '+FloatToStr(Total)+', 0 '+
        ' )';
        QTemp.ExecSQL;
        ...
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(fecha)+', '+QuotedStr('PAGO DE SERVICIO '+(ProveedorLabel.Caption))+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', '+Saldo+', 0 '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(fecha)+', '+QuotedStr('PAGO DE SERVICIO '+(ProveedorLabel.Caption))+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+MntCheque3ro+' '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(fecha)+', '+QuotedStr('PAGO DE SERVICIO '+(ProveedorLabel.Caption))+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+(MntCheque)+' '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(fecha)+', '+QuotedStr('PAGO DE SERVICIO '+(ProveedorLabel.Caption))+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+(MntContado)+' '+
        ' )';
        QTemp.ExecSQL;
        end;
...
begin
   ...
   try
      ...
      If BuscarImpuestosServiciosForm.Cod_ImpuestosServicios <> '' then Codigo:=BuscarImpuestosServiciosForm.Cod_ImpuestosServicios else Codigo:='0';
         PagosADOQuery.SQL.Text:='SELECT * FROM Pagos INNER JOIN ImpuestosServicios ON Pagos.CodImpuestoServicio = ImpuestosServicios.CodImpuestoServicio WHERE ImpuestosServicios.CodImpuestoServicio='+Codigo;
         PagosADOQuery.Open;
         CtaNombre:= BuscarImpuestosServiciosForm.ImpuestosServiciosQuery.FieldByName('CtaNombre').AsString;
         CtaTipo:= BuscarImpuestosServiciosForm.ImpuestosServiciosQuery.FieldByName('CtaTipo').AsString;
         CtaIIBB:= BuscarImpuestosServiciosForm.ImpuestosServiciosQuery.FieldByName('CtaIIBB').AsString;
         CtaAnticipo := BuscarImpuestosServiciosForm.ImpuestosServiciosQuery.FieldByName('CtaAnticipo').AsString;//CUENTA servicio
...
begin
      QTemp.SQL.Text:='SELECT "'+EmpresaQuery.FieldByName('Nombre').AsString+'" As Empresa, * FROM Pagos INNER JOIN ImpuestosServicios ON Pagos.CodImpuestoServicio = ImpuestosServicios.CodImpuestoServicio WHERE ImpuestosServicios.CodImpuestoServicio='+Codigo;
      QTemp.Open;
