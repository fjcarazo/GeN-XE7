...
uses
  ...
  Dialogs, DB, ADODB, StdCtrls, DBTables, ExtCtrls, IniFiles,
...
begin
 ...
 if (tabla<>'Factura') and (tabla<>'FacturaItem') and (tabla<>'Compra') and (tabla<>'CompraItem') then
 begin
 ...
 with tde do
  ...
  for n := 0 to tor.RecordCount-1 do
    ...
    qd.SQL.Text:='Select '+tor.Fields.Fields[0].DisplayName+' From '+tabla+' Where '+tor.Fields.Fields[0].DisplayName+' = '+QuotedStr(VarToStr(tor.Fields.Fields[0].Value))+''
    else if tor.Fields.Fields[0].DataType = ftAutoInc then
    qd.SQL.Text:='Select Fecha From '+tabla+' Where (Fecha = #'+DateToStr(tor.Fields.FieldByName('Fecha').AsDateTime)+'#)'
    else
    qd.SQL.Text:='Select '+tor.Fields.Fields[0].DisplayName+' From '+tabla+' Where '+tor.Fields.Fields[0].DisplayName+' = '+VarToStr(tor.Fields.Fields[0].Value)+'';
      ...
      if (Fields[i].AsString = '') and (Fields[i].DataType <> ftAutoInc) then
        ...
        if Assigned(tor.Fields[i]) then
              begin
                ...
                end else Fields[i] := tor.Fields[i];
              end;
            end;
        end;
    end;
...
if unidad = '' then ShowMessage('Verifique que este bien conectado el Pendrive. O que el sistema antiguo este copiado en el mismo');
end;
...
begin
conecta;
if unidad <> '' then  copiatodo;//Copiar;
end;
...
begin
  ...
  AssignFile(Origen,O);  //AssignFile(Origen,O+'\DeG\DistribuidoraCasadoro\DistribuidoraCasadoro.deg');
  reset(Origen);
...
begin
    if UnidadRadioGroup.ItemIndex  = 0 then CopiaTodo(BaseDeDatos,unidad) else CopiaTodo(unidad+'\'+DBase,BaseDeDatos);
    showmessage('Proceso terminado con éxito');
close;
end;
