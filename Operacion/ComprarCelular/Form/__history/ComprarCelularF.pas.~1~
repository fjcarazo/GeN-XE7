...
if cbTipo.ItemIndex = 0 then Impuesto:=IVA21+IVA105+IVAO;
...
procedure TComprarCelularForm.Quicksort(Grid:TStringGrid; var List:array of integer;
    min, max,sortcol,datatype: Integer);
...
procedure TComprarCelularForm.Sortgrid(Grid : TStringGrid; sortcol,datatype:integer);
var
   i : integer;
...
begin
   ...
   try
      ...
      begin
        ...
        Label23.Caption := DateToStr(IncDay(now,ATProve.FieldByName('DiasCredito').AsInteger));
        ...
        if (EmpresaQuery.FieldByName('CondicionIVA').AsString = 'RI') and (ATProve.FieldByName('IVA').AsString = 'RI') then cbTipo.ItemIndex := 0
        else if (ATProve.FieldByName('IVA').AsString = 'RI') then cbTipo.ItemIndex := 1
...
// ************ Boton de Agregar **************
 ...
 // Filtra las series que ya estan en la factura
   If (SGFact.RowCount = 2) AND (SGFact.Cells[2,1] = '') then SeleccionarCelularForm.Filtro := ''
   else
   begin
      ...
      begin
         SeleccionarCelularForm.Filtro := SeleccionarCelularForm.Filtro + ' AND NumSerie <> '+SGFact.Cells[2,i];
         i:=i+1;
      end;
   end;
   try
      ...
      Begin
         ...
         QTemp.Close;
         QTemp.SQL.Text := 'SELECT Descripcion+" "+Categoria+" "+SubCategoria AS Descr, * FROM Articulos '+
         ...
         SGFact.Cells[4,cuenta] := Format('%8.2f',[StrToFloat(SeleccionarCelularForm.FloatEdit1.Text) / (StrToFloat(SGFact.Cells[6,cuenta])/100+1) - StrToFloat(SGFact.Cells[2,cuenta]) ] );
         ...
         SGFact.Cells[9,cuenta] := QTemp.FieldByName('CodIngresosBrutos').AsString;//ACTIVIDAD DE MONOTRIBUTO
         q.SQL.Text:='SELECT * FROM IngresosBrutos WHERE CodIngresosBrutos='+QuotedStr(SGFact.Cells[9,cuenta]);
         q.Open;
...
begin
 ...
 // Filtra las series que ya estan en la factura
   ...
   begin
      ...
      While i < SGFact.RowCount do
      begin
         If i <> SGFact.Row then SeleccionarCelularForm.Filtro := SeleccionarCelularForm.Filtro + ' AND NumSerie <> '+SGFact.Cells[2,i];
         i:=i+1;
      end;
   end;
 //Carga los datos de la linea en el form
   ...
   try
      ...
      Begin
         ...
         SGFact.Cells[4,SGFact.Row] := Format('%8.2f',[StrToFloat(SeleccionarCelularForm.FloatEdit1.Text)]);
...
begin
      ...
      if StrToFloat(Saldo) > 0.04 then Tipo:= 'C' else  Tipo:= 'F';
   If FPagoOk then
   begin
//   MntContado:=FloatToStr( StrToFloat(Saldo)+StrToFloat(Saldo)+StrToFloat(Saldo));
  ...
  // Obtener el numero de Compra
     QTemp.SQL.Text := 'Select Max(CodCompra) From Compra where Tipo = '+QuotedStr(Tipo);
     QTemp.Open;
  ...
  // Iniciar la Transaccion
   ...
   // Insertar en la tabla de Compra si NO tiene ORDEN
        QTemp.SQL.Text := 'Insert Into Compra ( CodCompra, Secuencia, Tipo, Fecha, CodProve, Terminos, OrdenCompra, CodVendedor, Excento, TasaImpuesto, Anulada, MntEnvio, MntSubTotal, MntDescuento, MntImpuesto, MntTotal, '+
        'MntContado, MntCheque, MntTarjeta, MntOtros, Saldo, Pagado, TC, DetPago, Hora, Letra) Values '+
        '( '+IntToStr(NumFact)+', 0, '+QuotedStr(Tipo)+', '+QuotedStr(Fecha)+', '+Edit1.Text+', '+QuotedStr(Label19.Caption)+', '+QuotedStr(Edit4.Text)+', '+QuotedStr(Edit3.Text)+
        ', False, ''0.21'', False, ''0.00'', '+QuotedStr(SGTotal.Cells[1,0])+', '+QuotedStr(SGTotal.Cells[1,1])+', '+
        QuotedStr(SGTotal.Cells[1,2])+', '+QuotedStr(SGTotal.Cells[1,4])+', '+QuotedStr(MntContado)+', '+QuotedStr(MntCheque)+', '+
        QuotedStr(MntTarjeta)+', '+QuotedStr(MntOtros)+', '+QuotedStr(Saldo)+', '+Pagado+', '+QuotedStr(FloatToStr(0))+', '+QuotedStr('Cheque: '+DetCheque+' $'+MntCheque+' a días '+DiaCheque+', Tarjeta: '+DetTarjeta+' $'+MntTarjeta)+', '+QuotedStr(TimeToStr(Time))+', '+QuotedStr(cbTipo.Text)+')';
        QTemp.ExecSQL;
  ...
  // Insertar en la tabla de CompraItem
        For i := 1 to SgFact.RowCount-1 do
         ...
         if SGFact.Cells[0,i] <> '1' then detalle:= SGFact.Cells[1,i] else detalle := memo;
           ...
           SqL := 'Insert Into CompraItem Values ( '+IntToStr(numfact)+', 0, '+QuotedStr(Tipo)+', '+IntToStr(i)+', '+(SGFact.Cells[0,i])+', '+(SGFact.Cells[2,i])+', '+(SGFact.Cells[3,i])+', '+(SGFact.Cells[4,i])+', '+SGFact.Cells[6,i]+', '+IntToStr(numfact)+', '+QuotedStr(detalle)+');';
           QTemp.SQL.Text := SqL;
           QTemp.ExecSQL;
         end;
  ...
  // Insertar en la Tabla Caja
          ...
          begin
           ...
           ' ('+QuotedStr('Cheque')+', '+MntCheque+', '+QuotedStr(DateToStr(Date))+', '+QuotedStr(TimeToStr(Time))+')';
           QTemp.SQL.Text := SqL;
           QTemp.ExecSQL;
           end;
  ...
  // Insertar en la Tabla Caja
          ...
          begin
           SqL := 'Insert Into Caja (Detalle, Haber, Fecha, Hora) Values'+
           ' ('+QuotedStr('Efectivo')+', '+MntContado+', '+QuotedStr(DateToStr(Date))+', '+QuotedStr(TimeToStr(Time))+')';
           QTemp.SQL.Text := SqL;
           QTemp.ExecSQL;
           end;
  ...
  // Insertar en la tabla de CtaCte
          ...
          QTemp.Active := True;
          if QTemp.RecordCount > 0 then SqL := 'Update CtaCte Set Saldo = Saldo + '+floattostr(-strtoFloat(Saldo))+', Fecha = '+DateToStr(Date)+' Where CodProve = '+Edit1.Text
          else SqL := 'Insert Into CtaCte (CodProve, Fecha, Hora, Saldo) Values ('+Edit1.Text+','+DateToStr(Date)+','+QuotedStr(TimeToStr(Time))+','+floattostr(-strtoFloat(Saldo))+')';
          QTemp.Close;
           QTemp.SQL.Text := SqL;
           QTemp.ExecSQL;
           end;
  ...
  // Insertar en la tabla de CtaCte Item
        ...
        For i := 1 to SgFact.RowCount-1 do
         ...
         if SGFact.Cells[0,i] <> '1' then detalle:= SGFact.Cells[1,i] else detalle := memo;
           if SGFact.Cells[6,i] = 'Si' then iv:='True)' else iv:='False)';
           QTemp.SQL.Text := 'Insert Into CtaCteItem (CodProve, CodFactura, Linea, Fecha, Hora, CodParte, Detalle, Precio, Cantidad, Importe) Values'+
           '( '+Edit1.Text+','+IntToStr(NumFact)+','+IntToStr(i)+','+DateToStr(Date)+','+QuotedStr(TimeToStr(Time))+','+(SGFact.Cells[0,i])+','+QuotedStr(SGFact.Cells[1,i])+','+(SGFact.Cells[4,i])+','+(SGFact.Cells[3,i])+','+(SGFact.Cells[5,i])+')';
           QTemp.ExecSQL;
         end;
         end;
  ...
  // Insertar en la tabla de Servicios
          if memo <> '' then
          begin
           SqL := 'Insert Into Servicios Values ('+IntToStr(NumFact)+','+QuotedStr(memo)+','+DateToStr(Date)+')';
           QTemp.SQL.Text := SqL;
           QTemp.ExecSQL;
           end;
  ...
  // Actualizar la tabla de Articulos
        For i := 1 to SgFact.RowCount-1 do
         begin
         Q.SQL.Text := 'Select * From Articulos where  CodParte = '+(SGFact.Cells[0,i]);
         Q.Open;
           if Q.FieldByName('UltCosto').AsFloat < 0.00001 then //ultimo costo
           begin
           QTemp.SQL.Text := 'Update Articulos Set UltCosto = '+(SGFact.Cells[4,i])+' Where '+
           ...
           begin
           QTemp.SQL.Text := 'Update Articulos Set Disponible = Disponible + '+SGFact.Cells[3,i]+', Costo = '+(SGFact.Cells[4,i])+' Where '+
                             'CodParte = '+(SGFact.Cells[0,i]);
           QTemp.ExecSQL;
           end;
           Q.Close;
...
//++CONTABILIDAD ******************************************************************************************************************************************************************************************************
  ...
  // Insertar en el Libro IVA Compras
          ...
          begin
           ...
           ' ('+QuotedStr(DateToStr(Date))+', '+QuotedStr(TimeToStr(Time))+', '+QuotedStr(Tipo)+', '+IntToStr(NumFact)+', '+Edit1.Text+', '+QuotedStr(Label27.Caption)+', '+FloatToStr(NG105)+', '+FloatToStr(NG21
    ...
    //Insertar en la tabla LibroDiario
        ...
        q.Open;
        c:=c+1;
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('Compra Nº'+IntToStr(NumFact)+' - '+(Tipo)+' - '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', '+FloatToStr(SubTotal+flete-desc)+', 0 '+
        ' )';
        QTemp.ExecSQL;
        ...
        c:=c+1;
        q.SQL.Text:='select * from Cuenta where codigo='+ConfigQuery.FieldByName('CtaIVACreditoFiscal').AsString;
        q.Open;
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('Compra Nº'+IntToStr(NumFact)+' - '+(Tipo)+' - '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', '+FloatToStr(Impuesto)+', 0 '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('Compra Nº'+IntToStr(NumFact)+' - '+(Tipo)+' - '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+FloatToStr(Total-StrToFloat(Pagado))+' '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        c:=c+1;
        q.SQL.Text:='select * from Cuenta where codigo='+ConfigQuery.FieldByName('CtaObligacionAPagar').AsString;
        q.Open;
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('Compra Nº'+IntToStr(NumFact)+' - '+(Tipo)+' - '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+FloatToStr(Total-StrToFloat(Pagado))+' '+
        ...
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('SALDO A FAVOR CON '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', '+Saldo+', 0 '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        c:=c+1;
        q.SQL.Text:='select * from Cuenta where codigo='+ConfigQuery.FieldByName('CtaValorAlCobro').AsString;
        q.Open;
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('Compra Nº'+IntToStr(NumFact)+' - '+(Tipo)+' - '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+MntCheque3ro+' '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        c:=c+1;
        q.SQL.Text:='select * from Cuenta where codigo='+ConfigQuery.FieldByName('CtaBanco').AsString;
        q.Open;
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('Compra Nº'+IntToStr(NumFact)+' - '+(Tipo)+' - '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+(MntCheque)+' '+
        ' )';
        QTemp.ExecSQL;
        end;
        ...
        // renglon  - CAJA
        c:=c+1;
        q.SQL.Text:='select * from Cuenta where codigo='+ConfigQuery.FieldByName('CtaCaja').AsString;
        q.Open;
        QTemp.SQL.Text := 'Insert Into LibroDiario ( Codigo, Asiento, Fecha, Leyenda, Jerarquia, Cuenta, Debe, Haber)'+
        ' Values '+
        '( '+IntToStr(c)+', '+IntToStr(i)+', '+QuotedStr(Fecha)+', '+QuotedStr('Compra Nº'+IntToStr(NumFact)+' - '+(Tipo)+' - '+(ProveedorLabel.Caption)+' - '+(GerenteLabel.Caption)+'')+', '+QuotedStr(q.FieldByName('Jerarquia').AsString)+', '+QuotedStr(q.FieldByName('Cuenta').AsString)+', 0, '+(MntContado)+' '+
        ' )';
        QTemp.ExecSQL;
        end;
...
IF Key = VK_F4 then BitBtn2.Click;
IF Key = VK_F6 then BitBtn3.Click;
IF Key = VK_F8 then BitBtn4.Click;
IF Key = VK_Escape then BitBtn5.Click;
end;
