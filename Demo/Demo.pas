...
begin
  ...
  if ('2025-329D' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA KNOEPFLI CARLOS NORBERTO'
  else if ('0464-3080' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA LOPEZ JORGE ERNESTO'
  else if ('D803-4016' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA DAGOSTINO DIEGO'
    // else  if ('5CDC-43D4'= Maquina) then Licencia:= 'LICENCIA DE USO REGISTRADA PARA GUILLEN DIEGO'
  else if ('8095-B051' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA ABELLAN ANDRES'
  else if ('0AE0-8A6A' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA LOESCHBOR HERNAN'
  else if ('121C-6D80' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA MARTINEZ DE GIORGI DIEGO'
  else if ('A4D0-BDF6' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA VALDERREY HORACIO LUIS'
  else if ('A434-6A29' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA BOCCHIA VICENTE JULIO'
  else if ('C816-D617' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA BULONERIA SARMIENTO'
  else if ('8831-12CF' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA GALVAN MANCHINI PABLO EDUARDO'
  else if ('9496-F834' = Maquina) then
    Licencia := 'LICENCIA DE USO PARA JUAREZ CARPIO CESAR NICOLAS'
